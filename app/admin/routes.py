...
def dashboard():
    ...
    # Proteção caso service seja None
    ...
    appointments = Appointment.query.order_by(Appointment.start_datetime.desc()).limit(10).all()
...
def update_status(id, new_status):
    ...
    if new_status in ['confirmed', 'cancelled', 'completed']:
        ...
        db.session.commit()
        flash(f'Status de {appt.user.name} atualizado!', 'success')
...
def new_user():
    ...
    if request.method == 'POST':
        ...
        new_u = User(name=username, email=email)
...
def edit_user(id):
    ...
    if request.method == 'POST':
        ...
        if user.id == current_user.id and new_role == 'cliente':
            flash('Erro: Você não pode remover seu próprio acesso administrativo.', 'danger')
        else:
...
# --- GESTÃO DE SERVIÇOS (REVISADO PARA SAÚDE/ESTÉTICA) ---
...
def manage_service(id=None):
    ...
    resources = Resource.query.all() # Para vincular Salas/Equipamentos
    ...
    if request.method == 'POST':
        try:
            ...
            service.category = request.form.get('category') # RF003: Categoria
            ...
            service.resource_id = request.form.get('resource_id') # RF002: Equipamento/Sala
            ...
            db.session.rollback()
            flash('Erro ao salvar o serviço. Verifique os dados.', 'danger')
